...
export default async function changeAdminPassword(req: Request<{}, any, any, ParsedQs, Record<string, any>>, res: Response<any, Record<string, any>, number>) {
	...
	if (typeof req.body != 'object' || Object.keys(req.body).length != 3) {
		log('changeAdminPassword.ts', 'WARNING', 'change admin password has been call with wrong body');
		res.status(400).send({ message: "Specify { email: string, oldPassword: Sha512 string, newPassword: sha512String }" });
		return;
	}
	if (!req.body.email || typeof req.body.email != 'string') {
		log('changeAdminPassword.ts', 'WARNING', 'change admin password has been call with wrong type of email');
	...
	if (!req.body.oldPassword || req.body.oldPassword.length != 128 || !regexSHA512.test(req.body.oldPassword)) {
		log('changeAdminPassword.ts', 'WARNING', 'change admin password has been call with wrong type of oldPassword');
	...
	if (!admin) {
		log('changeAdminPassword.ts', 'WARNING', 'change admin password has been call but admin required does not exist');
		res.status(404).send({ message: 'Admin not found' });
		return;
	}
	if (admin.password != req.body.oldPassword) {
		log('changeAdminPassword.ts', 'WARNING', 'change admin password has been call with bad old password');
	...
	if (!req.body.newPassword || req.body.newPassword.length != 128 || !regexSHA512.test(req.body.newPassword)) {
		log('changeAdminPassword.ts', 'WARNING', 'change admin password has been call with bad type of newPassword');
	...
	log('changeAdminPassword.ts', 'INFORMATION', 'the ' + req.body.email + ' admin just changed his password');
